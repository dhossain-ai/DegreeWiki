...
Phase 67C - Program Import Bulk Update Existing Enrichment - complete.
The import batch page now includes a dedicated enrichment-pass action for programs. After a richer second research pass, admins can bulk update exact existing matches without creating duplicates: matching still requires normalized title + linked production university + degree level, only empty allowlisted production fields are filled, source URLs are attached best-effort with deduped URLs, ambiguous matches stay manual, and rows without exact matches are skipped. The batch page now explains that this flow is for filling missing fields only and does not publish or verify programs. Program `Update Existing` was also tightened to the explicit allowlist used by this phase. No migration, no new dependency, no unsafe HTML APIs, no service role, and no RLS bypass.
