...
Phase 68 - Import Pipeline Polish Bundle - complete.
The import UX now prioritizes the guided Program Import Staging path, moves General Batch Tools into an Advanced / Legacy role, adds type/status/active-recent filters to recent batches, and clarifies the batch-detail workflow as review rows, choose production action, then publish only if needed. Program first-import merge, enrichment update-existing, approve-only review, and publish-as-unverified behavior are now labeled directly in the UI without changing backend import semantics. Super-admins can delete old import batches from the batch detail page after checkbox + typed DELETE confirmation; cleanup removes only import/staging records and never deletes production programs, universities, scholarships, articles, media, or production data_sources. No migration, no new dependency, no unsafe HTML APIs, no service role, no RLS bypass, no merge/update/publish semantics change, and no auto-overwrite of non-empty production fields.
...
- Phase 68: import pipeline polish bundle; `/admin/imports` now makes Program Import Staging the primary recommended path, labels General Batch Creation as Advanced / Legacy, explains its non-standard use cases, adds recent-batch filters, and links directly to Program Import, active batches needing review, and batch history. `/admin/imports/programs` now spells out the five-step staging flow and first-import vs enrichment choices. `/admin/imports/[id]` now presents Step 1 review, Step 2 production action, Step 3 publish-only-if-needed, and Advanced tools, with contextual recommendations from cheap visible-row signals. Super-admin batch deletion removes import/staging records only after checkbox + typed DELETE confirmation. Production content and import backend semantics are unchanged.
- Phase 67C: program import bulk update existing enrichment; `/admin/imports/[id]` now adds a dedicated batch enrichment-pass panel for programs with selected-visible and all-matching scopes, each guarded by confirmation. Bulk enrichment reuses the exact duplicate-safe match rule (normalized title + linked production university + degree level), updates only empty allowlisted production fields, never creates production programs, skips no-match and ambiguous rows, and reports updated / no-exact-match / ambiguous / no-safe-fields / failed / source-warning counts in the batch summary. Program source URLs remain best-effort and deduped, and the batch-page helper copy plus import workflow docs now explain the second-pass enrichment workflow. No migration, no new dependency, no unsafe HTML APIs, no service role, and no RLS bypass.
- Phase 67B: program import final QA + prompt tightening; tightened the built-in program import prompt/template around supported fields, updated the preferred research-pack shape for university-by-university collection, clarified broad-subject/null guidance plus `source_urls`, explicitly excluded intake/deadline/status fields from new research JSON, updated helper copy on `/admin/imports/programs`, aligned generic batch helper text, and refreshed the import workflow docs with the recommended prompt, duplicate-safe re-import guidance, cleanup guidance, and a manual QA checklist. No migration, no new dependency, no unsafe HTML APIs, no service role, no RLS bypass, no `program_intakes` import, and no merge-semantics change.
- Phase 67A: program duplicate cleanup + safe delete; `/admin/programs` now supports a duplicate-only view via `?duplicates=1`, bulk row selection, archive selected, restore selected, and super-admin-only hard delete with a confirmation checkbox plus typed `DELETE`. Duplicate detection uses normalized title + university_id + degree_level_id from current production programs, hard delete cleans safe program-scoped rows (`scholarship_programs`, `data_sources`, `saved_items`, `user_reports`, `verification_events`, `data_quality_checks`) through existing RLS only, and skips programs referenced by AI Finder history or immutable analytics/outbound logs. `/admin/programs/[id]` now exposes matching archive/restore controls and a guarded hard-delete panel. Shared university rows and shared media assets are not deleted. No migration, no new dependency, no unsafe HTML APIs, no service role, and no RLS bypass.
