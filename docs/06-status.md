...
Phase 66E - Program Import Field Mapping Hardening - complete.
Program import staging and merge now harden the existing no-migration rich-field path without changing accepted JSON shapes or merge defaults. `mergeProgram()` still creates production programs as draft + unverified + indexing draft, still ignores JSON `content_status` / `verification_status`, and still does not create `program_intakes`. The import flow now recognizes additional safe aliases for rich program fields, warns on invalid optional enum-like values, warns when primary subjects are unknown or ambiguous, allows exact case-insensitive subject-name matches plus safe slug fallback, improves staged program previews, and keeps best-effort `data_sources` attachment with friendly failure reporting. No migration, no new dependency, no unsafe HTML APIs, no service role, no RLS bypass, no subject auto-creation, and no intake/deadline import in this phase.
