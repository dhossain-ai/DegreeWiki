...
Phase 67A - Program Duplicate Cleanup + Safe Delete - complete.
`/admin/programs` now has a duplicate-cleanup view keyed by normalized title + university + degree level, row selection, bulk archive, bulk restore, and super-admin-only hard delete with explicit confirmation. `/admin/programs/[id]` now adds matching archive/restore actions plus a guarded hard-delete section. Hard delete uses existing SSR Supabase auth/RLS only, never uses a service role, never deletes shared university rows or shared media assets, cleans only safe program-scoped auxiliary rows it can remove through existing policies, and skips records referenced by AI Finder history or immutable analytics/outbound logs with an archive recommendation instead. No migration, no new dependency, no unsafe HTML APIs, no service role, and no RLS bypass.
