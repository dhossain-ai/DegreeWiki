...
Phase 67B - Program Import Final QA + Prompt Tightening - complete.
The program import flow now has tighter built-in research prompts/templates, clearer importer-aligned helper copy, and updated workflow docs for university-by-university collection. Recommended JSON now stays closer to the importer's supported fields, explicitly avoids intake/deadline import, clarifies broad-subject/null rules, clarifies `source_urls`, and makes it explicit that server logic still creates draft/unverified production programs. The dedicated `/admin/imports/programs` page now explains the recommended flow, duplicate-safe re-import behavior, empty-field-only `Update Existing`, and later publish-as-unverified review flow. No migration, no new dependency, no unsafe HTML APIs, no service role, and no RLS bypass.
